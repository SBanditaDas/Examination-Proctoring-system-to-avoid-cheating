...
function App() {
  ...
  const [terminated, setTerminated] = useState(null); // { reason, timestamp, incidentId }
  ...
  // 🔒 Tab-Switch / Focus-Loss Lockdown — active only during exam
  const handleLockdownViolation = useCallback((violation) => {
    const id = `#${Math.floor(1000 + Math.random() * 9000)}-X`;
    setTerminated({
      reason: violation.type === 'TAB_SWITCH_DETECTED' ? 'Tab Switch Detected' : 'Window Focus Lost',
      timestamp: violation.timestamp,
      incidentId: id,
    });
  }, []);

  const isExamActive = !!baselineEmbedding && !examEnded && !terminated;
  useLockdown(handleLockdownViolation, isExamActive);
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
      {/* ========== TERMINATED SCREEN ========== */}
      {terminated ? (
        <div className="bg-slate-900 border border-slate-700 rounded-xl p-10 max-w-md w-full text-center shadow-2xl animate-in fade-in zoom-in duration-500">
          {/* Red Shield Icon */}
          <div className="mx-auto mb-6 w-20 h-20 rounded-full bg-red-500/15 flex items-center justify-center">
            <ShieldAlert className="w-10 h-10 text-red-500" />
          </div>

          {/* Heading */}
          <h1 className="text-3xl font-extrabold text-white mb-3">Exam Terminated</h1>
          <p className="text-slate-400 mb-8 leading-relaxed">
            Your session was automatically terminated due to a security violation (Tab switch or focus loss detected).
          </p>

          {/* Incident Report Card */}
          <div className="bg-slate-800 border border-slate-700 rounded-lg p-5 text-left mb-8">
            <div className="flex items-center justify-between mb-4">
              <span className="text-slate-500 text-xs font-semibold uppercase tracking-widest">Incident Report</span>
              <span className="text-slate-500 text-xs font-mono">ID: {terminated.incidentId}</span>
            </div>
            <p className="text-red-400 font-semibold mb-1">Violation: {terminated.reason}</p>
            <p className="text-slate-500 text-sm">Timestamp: {terminated.timestamp}</p>
          </div>

          {/* Return Button */}
          <button
            onClick={() => window.location.reload()}
            className="w-full py-4 bg-white text-slate-900 font-bold text-sm uppercase tracking-widest rounded-lg hover:bg-slate-200 transition-colors"
          >
            Return to Dashboard
          </button>
        </div>
